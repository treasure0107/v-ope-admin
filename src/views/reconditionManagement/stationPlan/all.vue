 ...
 * @Author: JTune
 * @Date: 2021-08-13
 * @LastEditTime: 2021-12-29 11:44:40
 * @LastEditors: JTune
        ...
        <el-header style="height:45px;">
          <el-row type="flex" justify="space-between" align="middle" style="height:100%">
            <el-col style="width:unset;">
              ...
              切换场站：<el-select v-model="checkStation" placeholder="请选择场站" size="mini" @change="(val)=>{getDeviceList(val)}">
                <el-option
...
export default {
  ...
  methods: {
      ...
      this.$axios.post(
        ...
        res => {
          if (res.isSuccess && res.data && res.data.length) {
            ...
            this.getDeviceList(res.data[0].id);
          } else {
    ...
    getDeviceList(id) {
      this.tableData = [];
      this.$axios.post(
        Api.stationManagement.getStationList,
        {
          data: { node: { id }}
        },
        res => {
      ...
      this.$refs.personForm.validate((valid) => {
        if (valid) {
          ...
          if (this.toEditTableItem) {
            console.log(this.personForm, 'personForm-personForm');
    ...
    devideUpdataHandle() {
      this.$axios.post(
        Api.stationManagement.stationUpdate,
        {
          data: {
    ...
    createHandle() {
      ...
      this.$axios.post(
        Api.stationManagement.stationSave,
        {
          data: {
            node: { id: this.$store.state.user.stationId },
            user: { id: this.personForm.user },
    ...
    deleteClick() {
      ...
      this.$confirm('确认要删除吗?', '提示', {
        ...
        this.$axios.post(
          Api.stationManagement.stationDelete,
          {
